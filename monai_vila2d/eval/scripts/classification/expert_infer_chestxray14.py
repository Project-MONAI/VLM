...
def batch_run(exp_id=0, mpath=None, conv_mode=None, folder_name="", p_mode="binary"):
    ...
    if p_mode == "multi_choice":
        out_csv = os.path.join(f"{folder_name}", "test_vila_prompt.csv")
        choices = "\n".join(f"({c}) {cls[k]}" for c, k in zip("ABC", cls.keys()))
        choices += "\n(D) none of the above\n"
        ...
        out_csv = os.path.join(f"{folder_name}", f"test_vila_{cls[exp_id].lower().replace(' ', '_')}.csv")
    ...
    expert_base = "/data/datasets/chestxray14/torchxrayvision/"
    ...
    for idx, line in enumerate(lines[1:]):  # skip the header
        ...
        if not has_placeholder(prompt, "<placeholder>"):
            ...
            for expert_res in expert_models:
                json_path = os.path.join(expert_base, expert_res, fname.replace("jpg", "json"))
